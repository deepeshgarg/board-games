...
//%s/this.moveflagPromoteBishop/this.moveflagPromoteBishop/g
// Perf TODO:
// Merge material updating with psq values
// Put move scoring inline in generator
// Remove need for this.flipTable in psq tables.  Access them by color
...
function GarboChess() {
    ...
    this.g_baseEval;
    this.g_hashKeyLow, this.g_hashKeyHigh;
    this.g_inCheck;
...
GarboChess.prototype.GetMoveSAN = function(move, validMoves) {
    ...
    } else if (pieceType == this.piecePawn && this.g_board[to] != 0) {
        result += this.FormatSquare(from).charAt(0);
    }
...
GarboChess.prototype.GetMoveFromString = function(moveString) {
    ...
    var formattedMove = this.FormatMove(moves[i]);
    console.log(formattedMove);
        if (this.FormatMove(moves[i]) == moveString) {
            return moves[i];
        }
    }
    console.log("busted! ->" + moveString + " fen:" + this.GetFen());
}

GarboChess.prototype.GetMoveFromSAN = function(moveString) {
...
function MovePicker (garboChess, hashMove, depth, killer1, killer2) {
    this.garboChess = garboChess;
    this.g_board = garboChess.g_board;
    ...
    this.nextMove = function() {
        if (++this.atMove == this.moveCount) {
            this.stage++;
            if (this.stage == 1) {
                if (this.hashMove != null && this.garboChess.IsHashMoveValid(hashMove)) {
            ...
            if (this.stage == 2) {
                this.garboChess.GenerateCaptureMoves(this.moves, null);
                this.moveCount = this.moves.length;
                this.moveScores = new Array(this.moveCount);
                // Move ordering
                for (var i = this.atMove; i < this.moveCount; i++) {
                    var captured = this.g_board[(this.moves[i] >> 8) & 0xFF] & 0x7;
                    var pieceType = this.g_board[this.moves[i] & 0xFF] & 0x7;
            ...
            if (this.stage == 3) {
                if (this.garboChess.IsHashMoveValid(this.killer1) &&
            ...
            if (this.stage == 4) {
                if (this.garboChess.IsHashMoveValid(this.killer2) &&
            ...
            if (this.stage == 5) {
                this.garboChess.GenerateAllMoves(this.moves);
                this.moveCount = this.moves.length;
                // Move ordering
                for (var i = this.atMove; i < this.moveCount; i++) this.moveScores[i] = this.garboChess.ScoreMove(this.moves[i]);
                // No moves, onto next stage
                if (this.atMove == this.moveCount) this.stage++;
            }
        ...
        if (this.stage == 2 && !this.garboChess.See(candidateMove)) {
            if (this.losingCaptures == null) {
...
GarboChess.prototype.InitializeEval = function() {
    ...
    for (var i = 0; i < 2; i++) {
        ...
        this.g_mobUnit[i][enemy | this.pieceBishop] = 1;
        this.g_mobUnit[i][enemy | this.pieceKnight] = 1;
        this.g_mobUnit[i][enemy | this.pieceRook] = 1;
        this.g_mobUnit[i][enemy | this.pieceQueen] = 1;
        this.g_mobUnit[i][enemy | this.pieceKing] = 1;
...
GarboChess.prototype.InitializeFromFen = function(fen){
        ...
        if (c == '/') {
            ...
            if (c >= '0' && c <= '9') {
                for (var j = 0; j < parseInt(c); j++) {
                    this.g_board[((row + 2) * 0x10) + (col + 4)] = 0;
                    col++;
                }
            }
            else {
                ...
                this.g_board[((row + 2) * 0x10) + (col + 4)] = piece;
                col++;
            }
        }
    }
    ...
    if (chunks[3].indexOf('-') == -1) {
        var fixEnPassant = this.fixEnPassantSquare(chunks[3]);
        this.g_enPassentSquare = parseInt(fixEnPassant, 16);
    }
    ...
    this.g_move50 = 0;
    this.g_inCheck = this.IsSquareAttackable(this.g_pieceList[(this.g_toMove | this.pieceKing) << 4], 8 - this.g_toMove);
}


GarboChess.prototype.fixEnPassantSquareMap = {
    "a6" : "44", "b6" : "45", "c6" : "46", "d6" : "47", "e6" : "48", "f6" : "49", "g6" : "4a", "h6" : "4b",
    "a3" : "74", "b3" : "75", "c3" : "76", "d3" : "77", "e3" : "78", "f3" : "79", "g3" : "7a", "h3" : "7b"
}

GarboChess.prototype.fixEnPassantSquare = function(sqr) {
    if (typeof(this.fixEnPassantSquareMap[sqr]) != "undefined") {
        return this.fixEnPassantSquareMap[sqr]; 
    }
    return sqr;
}
...
// Test Harness
//////////////////////////////////////////////////
GarboChess.prototype.FinishPlyCallback = function(bestMove, value, timeTaken, ply) {
    postMessage("pv " + BuildPVMessage(bestMove, value, timeTaken, ply));
}

GarboChess.prototype.FinishMoveLocalTesting = function(bestMove, value, timeTaken, ply) {
    if (bestMove != null) {
...
self.onmessage = function(e) {
    ...
    if (e.data.match("^position") == "position") {
        ...
        this.g_timeout = parseInt(e.data.substr(7, e.data.length - 7), 10);
        Search(FinishMoveLocalTesting, 99, FinishPlyCallback);
    } else if (e.data == "analyze") {
        this.g_timeout = 99999999999;
        Search(null, 99, FinishPlyCallback);
    } else {
        this.MakeMove(GetMoveFromString(e.data));
    }
}
